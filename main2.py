...
# --- 初始化天文引擎 ---
# 請確保目錄下有 'de421.bsp' 文件，否則 skyfield 會報錯
...
def get_ming_gong(zhi_yue, h_zhi):
    """計算命宮：(14 或 26) - (月支數 + 時支數)"""
    # zhi_yue: 1=寅, 2=卯... 確保傳入的是術數月數
    # h_zhi: 時支
...
def get_day_basic_data(dt_date):
    ...
    zhi_yue = int(shifted_lon // 30) + 1 # 這裡計算月支數 (寅=1)
...
def run_pro_calendar(start_str, days):
    ...
    for i in range(days):
        ...
        for idx, name, period, is_late in time_slots:
            ...
            # 計算時星：晚子時需要用到明天的日支與陰陽遁 (修正邏輯)
            h_s = get_hour_star_pro(
                d["is_yang"], d["day_zhi"], idx, 
                is_late_rat=is_late, 
                next_day_is_yang=d_next["is_yang"], 
                next_day_zhi=d_next["day_zhi"]
            )
            ...
            rows.append({
                ...
                "年柱": d["y_gz"], "年納音": NAYIN.get(d["y_gz"]),
                "月柱": d["m_gz"], "月納音": NAYIN.get(d["m_gz"]),
                "日柱": d["d_gz"], "日納音": NAYIN.get(d["d_gz"]),
                "時柱": h_gz, "時納音": NAYIN.get(h_gz),
                "胎元": get_tai_yuan(d["m_gz"]),
                "命宮": get_ming_gong(d["zhi_yue"], h_gz[1]),
                "年星": d["y_s"], "月星": d["m_s"], "日星": d["d_s"],
...
if __name__ == "__main__":
    # 設定為 1978 年開始
    df = run_pro_calendar("1978-01-01", 365)
    
    # 匯出檔案
    df.to_csv("calendar_1978_full_features.csv", index=False, encoding='utf_8_sig')
    df.to_excel("calendar_1978_full_features.xlsx", index=False, engine='openpyxl')
    
    print("✅ 1978年曆法生成完畢 (包含早晚子時修正、納音、胎元、命宮)。")
    print(df.head(13).to_string()) # 預覽 1978-01-01 的完整 13 個時段